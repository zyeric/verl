...
from verl.workers.actor.nnscaler_actor import NNScalerPPOActor
...
class ActorRolloutRefWorker(NNScalerWorker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        set_random_seed(seed=self.config.actor.nnscaler.seed)
        ...
        if self._is_actor and self._is_rollout:
            ...
            self.config.actor.ppo_mini_batch_size //= self.data_parallel_world_size
            if self.config.actor.get("ppo_micro_batch_size", None):
                self.config.actor.ppo_micro_batch_size //= self.data_parallel_world_size
                self.config.rollout.log_prob_micro_batch_size //= self.data_parallel_world_size
                self.config.actor.ppo_micro_batch_size_per_gpu = self.config.actor.ppo_micro_batch_size
                self.config.rollout.log_prob_micro_batch_size_per_gpu = self.config.rollout.log_prob_micro_batch_size

            self._is_offload_param = self.config.actor.nnscaler.get("param_offload", False)
            self._is_offload_grad = self.config.actor.nnscaler.get("grad_offload", False)
            self._is_offload_optimizer = self.config.actor.nnscaler.get("optimizer_offload", False)
        elif self._is_ref:
            if self.config.ref.get("log_prob_micro_batch_size", None):
                self.config.ref.log_prob_micro_batch_size //= self.data_parallel_world_size
...
class CriticWorker(NNScalerWorker, DistProfilerExtension):
...
class RewardModelWorker(NNScalerWorker, DistProfilerExtension):
    """
