...
from verl.single_controller.base.nnscaler.worker import DistGlobalInfo, DistRankInfo
...
class NVNNScalerRayWorkerGroup(RayWorkerGroup, NNScalerWorkerGroup):
    """
    NNScalerWorkerGroup will query each worker of its nnscaler rank info and store it inside the WorkerGroup
    ...
    def __init__(self, resource_pool: RayResourcePool, ray_cls_with_init: RayClassWithInitArgs, **kwargs):
        ...
        self._nnscaler_rank_info: DistRankInfo = self.execute_all_sync(method_name="get_nnscaler_rank_info")
        self._nnscaler_global_info: DistGlobalInfo = ray.get(self.execute_rank_zero_async(method_name="get_nnscaler_global_info"))
